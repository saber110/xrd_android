...
class BuildingInfo(BaseModel):
    ...
    overheadLayer = db.Column(db.Text, server_default='_', comment='架空层')
    aboveGroundGarage = db.Column(db.Text, server_default='_', comment='地上车库')
    utilityRoom = db.Column(db.Text, server_default='_', comment='杂物间')
    residential = db.Column(db.Text, server_default='_', comment='住宅')
    shop = db.Column(db.Text, server_default='_', comment='商铺')
    ...
    unitName = db.Column(db.Text, server_default='无', comment='单元名称')
    roomName = db.Column(db.Text, server_default='室', comment='室号名称')
    ...
    mainTowards = db.Column(db.Text, server_default='南', comment='主要朝向')
    planeLayout = db.Column(db.Text, server_default='一般', comment='平面布局')
    completedYear = db.Column(db.Integer, comment='建成年份')
    decorationStandard = db.Column(db.Text, server_default='毛胚房', comment='装修标准')
    decorationYear = db.Column(db.Integer, comment='装修时间')
    buildingProperty = db.Column(db.Text, server_default='商品房', comment='房产性质')
    buildingStatus = db.Column(db.Text, server_default='存量', comment='楼栋状态')
    roofTopInfo = db.Column(db.Text, server_default='平层', comment='顶楼情况')
    roofInfo = db.Column(db.Text, server_default='平屋顶', comment='屋面情况')
    roofTopTerrace = db.Column(db.Text, server_default='无', comment='有无顶楼露台')
    roofTopAttic = db.Column(db.Text, server_default='无', comment='有无顶楼阁楼')
    roofTopLayer = db.Column(db.Text, server_default='无', comment='有无顶楼跃层')
